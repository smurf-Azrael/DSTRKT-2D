...
export function Navbar({ routes, action, theme }) {
	...
	const { openNav, updateNav } = useContext(MyContext);
	const { isEnd, updateEnd } = useContext(MyContext);

	const navigate = useNavigate();

	const handleOpenDribble = (title) => {
		if (title === 'home') {
			updateNav(false);
			updateEnd(true);
		...
		<ul className="lex-col mb-4 mt-2 flex gap-2 text-inherit lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
			{routes.map(({ title }) => (
				<Typography
					...
					onClick={() => handleOpenDribble(title)}
				>
		...
		<MTNavbar color="transparent" className="mx-auto p-3">
			<div className="mx-auto flex w-full items-center justify-between text-white">
				...
				<IconButton
					variant="outlined"
					size="lg"
					className="z-20 rounded-full border-none bg-[#C9F24F] text-black lg:hidden"
					onClick={() => updateNav(!openNav)}
				>
					{openNav ? (
			...
			>
				<div className="mx-2 my-4">
					<IconButton
						variant="outlined"
						size="lg"
						className="z-20 text-black ml-2 my-4 rounded-full border-none bg-[#C9F24F] lg:hidden"
						onClick={() => updateNav(!openNav)}
					>
						<XMarkIcon strokeWidth={2} className="h-6 w-6" />
					</IconButton>
					<div className="absolute h-screen w-full top-0 left-0 flex flex-col items-center justify-center gap-3">
						{routes.map(({ title }) => (
							<div key={title} className='uppercase text-black text-center text-[1.5rem] md:text-[2rem]' onClick={() => handleOpenDribble(title)}>
								{title}
							</div>
						))}
